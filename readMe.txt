...
- App.java
This class is the main class of the program where it processes command line args, calls the read file method, selects which algorithm to sort a selected shape dimention. It then prints the output of the sorted array and the time it took to execute.

- ReadFileClass.java
This class has the file reading logic for the project. We assume the text file only has one line. The class itterates over the text file, sorts and creates shape objects, then adds them to Shape[] shapeArray.
...
- Shape.java
This class is the parent class of the different shape classes. It has a constructor for shape, getter and setter methods for each dimention, overrides for compare and compareTo methods, and a toString method.
